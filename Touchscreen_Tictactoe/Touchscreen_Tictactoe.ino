...
void loop()
{
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    p.x = map(p.x, TS_MINX, TS_MAXX, 0 ,w);
    p.y = map(p.y, TS_MINY, TS_MAXY, 0, h);

    Serial.print("X = "); Serial.print(p.x);
    Serial.print("\tY = "); Serial.print(p.y);
    Serial.print("\n");

    if (p.x > 40 && p.x < 60 && p.y > 80 && p.y < 250 && buttonEnabled) // The user has pressed inside the red rectangle
    {
...
void playerMove()
{
  ...
  do
  {
    ...
    if (p.z > MINPRESSURE && p.z < MAXPRESSURE)
    {
      ...
      else if ((p.x > A && p.x <= B) && (p.y > C && p.y <= D)) //4
      {
        if (board[4] == 0)
        {
